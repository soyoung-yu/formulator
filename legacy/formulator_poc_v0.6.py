"""
AI 기반 에센스 처방 자동 생성 시스템 — PoC v0.6
================================================
v0.6 변경 내용: v0.5 대비
  1. 슬롯 기반 질의 해석 추가
     - Claude가 사용자 질의에서 성분 / 제형 / 마케팅 포인트를 먼저 JSON으로 구조화
     - 전체 문장, 부분 중복 표현, 숫자 조각을 후처리에서 제거
     - 제형/마케팅 포인트는 키워드 매칭, aspect 선정, 처방 프롬프트 지침에 간접 반영

  2. 성분명 탐색 파이프라인 추가
     - 성분 슬롯만 ConceptExpander 입력으로 사용해 query 전체/토큰/n-gram 반복 탐색 방지
     - 1단계: 정규화 exact match (띄어쓰기 제거 + 소문자화, 특수문자는 유지)
     - 2단계: 임베딩 유사도(보수적 임계값 0.90)
     - 3단계: Claude 소규모 호출 → 후보 성분명 생성 → data.csv 화이트리스트 교차 검증
     - 단계별 성분명 탐색 로그 출력

  3. 처방 성분 화이트리스트 방어 강화
     - 프롬프트에 허용 성분 전체 목록을 포함
     - 시스템 프롬프트에 목록 외 성분 사용 금지 명시
     - validate_and_fix()에서 미등록 성분을 IngredientMapper로 교체(score≥0.85)하거나 경고 태그 부착

  4. 프롬프트/컨텍스트 구성 개선
     - 추출된 맥락 표현, DB 내 최종 반영 성분, 소비자 체감 aspect를 프롬프트에 명시
     - 활성·기능성 성분 통계는 처방 건수 많은 순으로 정렬
     - Bedrock 클라이언트를 조기 생성해 슬롯 추출, 성분명 탐색, 처방 생성을 공유

  5. 결과 저장/추적성 개선
     - formula_output.xlsx에 Claude_프롬프트 시트 추가(system/user prompt, model_id, region 저장)
     - 프롬프트/설계 근거 등 긴 텍스트 셀은 엑셀 줄바꿈과 열 너비 적용
     - CSV/Excel/통계 JSON 저장 구조는 유지하면서 저장 helper 중복 정리

전체 흐름:
    입력 질의 → 슬롯 추출 → 성분명 탐색 → 통계/마케팅 컨텍스트 구성 →
    Claude 처방 생성 → 화이트리스트 검증/함량 보정 → 콘솔/CSV/Excel 저장

Usage:
    python formulator_poc_v0.6.py \\
        --data data.csv \\
        --product product.csv \\
        --query "초저분자 히아루론산과 비타민 C가 포함된 안티에이징 에센스"

    python formulator_poc_v0.6.py ... --model us.anthropic.claude-haiku-4-5-20251001-v1:0
...
QUERY_CONTEXT_KEYS = ("ingredients", "formulation", "marketing_points")
...
def extract_query_contexts(query: str, bedrock_client=None, model_id: str = "") -> dict[str, list[str]]:
    ...
    prompt = (
        "다음 화장품 제품 요구사항 문장에서 맥락 표현을 3개 카테고리로만 구조화하세요.\n"
        "반드시 JSON만 반환하세요.\n\n"
        "[카테고리 정의]\n"
    ...
    for key in QUERY_CONTEXT_KEYS:
        ...
        if not isinstance(values, list):
            values = []
        result[key] = _dedupe_context_phrases(values)
...
def _pick_similar(formula_dict, query_ing_names, priority_codes=None, n=3):
...
# ─────────────────────────────────────────────────────────────────────────────
# 9. AWS Bedrock 클라이언트 & Claude API 호출
# ─────────────────────────────────────────────────────────────────────────────
...
def run_pipeline(
    ...
    console.rule("AI 기반 에센스 처방 자동 생성 PoC v0.6 (AWS Bedrock / SageMaker)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="에센스 처방 자동 생성 PoC v0.6 (AWS Bedrock / SageMaker)")
