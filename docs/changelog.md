...
- **처방 생성 max_tokens 조정** (`llm.py`): 처방 3안 생성 호출의 출력 한도를 4096 → 8192 → 5500으로 조정. 긴 JSON 응답으로 인한 timeout 가능성을 줄이기 위한 제한.
- **시스템 프롬프트 출력 길이 제한 추가** (`prompt.py`): 전체 JSON 응답, Formula별 성분 수, role 길이, concept, design_rationale 길이 제한을 추가.

### 실행 요약 출력 강화

- **모델·시간 출력 추가** (`pipeline.py`, `llm.py`, `output.py`): 마지막 비용 요약에 사용 모델, 전체 실행 시간, 처방 생성 LLM 응답 시간을 함께 표시.

### Bedrock 요금표 보완

- **Claude Sonnet 4.6 요금 등록** (`config.py`): `anthropic.claude-sonnet-4-6`, `global.anthropic.claude-sonnet-4-6`를 입력 $3 / 출력 $15 per 1M tokens 기준으로 추가.

### 기본 모델 변경

- **DEFAULT_MODEL_ID 변경** (`config.py`): 기본 Bedrock 모델을 Claude 3.5 Sonnet에서 `global.anthropic.claude-sonnet-4-6`로 변경.
